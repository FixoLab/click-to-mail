    ...
    $.fn.togglebutton = function(opts) {
        ...
        this.each(function() {
            ...
            var buttons = options.map(function(index, opt) {
                var button = $(`<button type='button' class='btn__effect btn__effect${index}'><span></span></button>`)
                .prop('value', opt.value)
                .text(opt.text);

    
                // let ctm = document.querySlector('.ctm')
            ...
            var btnGroup = $("<div class='d-flex justify-content-center gap-2 flex-wrap'>").append(buttons);
